...
function visualiseTiles(cell,number,x,y,){
  ...
  celldiv.style.setProperty("--background-lightness",`${backgroundLightness}%`)
  celldiv.style.setProperty("--text-lightness",`${backgroundLightness <= 50 ? 90 : 10}%`)
}
...
function moveToright(row){
  ...
  for(let a = GRID_SIZE-2; a>=0; a--){
    ...
    if(tttrm.x===row[a].x+1&&tttrm.number!==row[a].number&&tttrm.number!=='') {
      tttrm = row[tttrm.x-1]
       continue
      }
...
function movetoup(column,board){
  ...
  for(let a = 1; a<=board.length-1; a++){
    ...
    if(tttrm.y===column[a].y-1&&tttrm.number!==column[a].number&&tttrm.number!=='') {
